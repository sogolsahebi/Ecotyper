...
This section will list and display all `.rda` files from the `data_all` and `data_sex` directories, covering a total of 57  study cohorts.
...
data_files_df <- data.frame(Cohorts = basename(data_files))
kable(data_files_df, caption = "All studies") %>%
...
summary_table <- data.frame(
  ...
  Time_to_event_outcome = character(),
  Response_outcome = character(),
  Patients = integer(),
...
for (file in data_files) {
  ...
  # Generate time_to_event_outcome
  time_to_event_outcome <- paste(
    ifelse(unique(clin$survival_type) == "None", "NA", unique(clin$survival_type)),
    collapse = ", "
  )

  # Generate Response_outcome (counts for R, NR, and NA)
  response_outcome <- paste0(
    "NR: ", sum(clin$response == "NR", na.rm = TRUE), ", ",
    "R: ", sum(clin$response == "R", na.rm = TRUE),
    if (sum(is.na(clin$response)) > 0) paste0(", NA: ", sum(is.na(clin$response))) else ""
  )
  ...
  summary_table <- rbind(summary_table, data.frame(
    ...
    Time_to_event_outcome = time_to_event_outcome,
...
# write.csv(summary_table, file = file.path(Base_dir, "outputs/Dataprocessing_outputs", "Summary_of_Each_IOdataset.csv"), row.names = TRUE)
```
...
- Keep only treatments `"PD-1/PD-L1"`, `"IO+combo"`, or `"CTLA4"`. Exclude treatments `"IO+chemo"`, `"IO+targeted"`, `"chemo"`, `"chemo+targeted"`, and `"targeted"`.  
- Exclude datasets with fewer than 25 patients.  
- Exclude the cancer type `"Lymph_node"` 
- Include only `"PRE"` samples :from `"Gide"`, `"Riaz"`, and `"Abacus"`.  
- Include only patients with RNA-seq data.  

```{r Table 1 }
# Note: Remove "Lymph_node" since "only keeping PRE samples" covers that - (is samples in ICB_Mariathasan__Lymph_node__PD-(L)1.rda , has pre/post samples? why this cover?)
...
# Subset only patients with RNA-seq data , RNA-seq data types: "rnaseq", "tpm", "fpkm"
...
# Note: Why keep both death and alive? follwoing same table for insight(for OS, death and alive)
...
generate_all_data <- function(data) {
  ...
  Characteristic <-c(
    ...
    "PFS (months)", "No-progression (%)", "Progressed (%)", "NA (%)",
    "Median PFS", "IQR PFS", "OS (months)", "Alive (%)", "Dead (%)", "NA (%)",
    "Median OS", "IQR OS")
  
  # Note:line 288: combined_clin_sex_before does not have column named "study"- no error foud
...
# Clean invalid UTF-8 characters for all columns
...
```{r Pie charts , width = 9, height = 5.1}
...
# other is (Esophageal, Colon, Lymph_node, Unknown, HNC, Live)
...
# Time to evennt in third layer
generate_sankey_tte <- function(data) {
  # Calculate total patients
  total_patients <- sum(data$Patients, na.rm = TRUE)
  
  # Summarize data for Sankey diagram
  summary_sankey <- data %>%
    group_by(Cancer_Type, Treatments, Time_to_event_outcome) %>%
  ...
  nodes <- data.frame(
    name = c(
      ...
      paste0(unique(summary_sankey$Time_to_event_outcome), "\n(n=", 
             sapply(unique(summary_sankey$Time_to_event_outcome), function(teo) sum(summary_sankey$Total_Patients[summary_sankey$Time_to_event_outcome == teo])), 
             ", ", 
             round(sapply(unique(summary_sankey$Time_to_event_outcome), function(teo) 
               sum(summary_sankey$Total_Patients[summary_sankey$Time_to_event_outcome == teo]) / total_patients * 100), 2), 
             "%)")
    )
  )
  
  # Prepare Links
  # Total Patients to Cancer Types
  links <- summary_sankey %>%
    group_by(Cancer_Type) %>%
    summarise(Total_Patients = sum(Total_Patients)) %>%
    mutate(
      source = 0, # Total Patients node
  ...
  # Treatments to Time to Event Outcome
  links <- links %>%
    bind_rows(
      summary_sankey %>%
        mutate(
          source = match(paste0(Treatments, "\n(n=", 
                                ...
                                round(sapply(Treatments, function(t) 
                                  sum(summary_sankey$Total_Patients[summary_sankey$Treatments == t]) / total_patients * 100), 2), 
                                "%)"), nodes$name) - 1,
          target = match(paste0(Time_to_event_outcome, "\n(n=", 
                                sapply(Time_to_event_outcome, function(teo) sum(summary_sankey$Total_Patients[summary_sankey$Time_to_event_outcome == teo])), 
                                ", ", 
                                round(sapply(Time_to_event_outcome, function(teo) 
                                  sum(summary_sankey$Total_Patients[summary_sankey$Time_to_event_outcome == teo]) / total_patients * 100), 2), 
                                "%)"), nodes$name) - 1,
          value = Total_Patients
        ) %>%
  ...
  # Return the Sankey Diagram for visualization
  return(sankey)
}

# Sankey diagram using Time_to_event_outcome as the third layer
sankey_time_to_event_before <- generate_sankey_tte(summary_table)
sankey_time_to_event_after <- generate_sankey_tte(summary_table_after)

# Visualize the Sankey diagrams
sankey_time_to_event_before 
sankey_time_to_event_after

# save the Sankey diagrams to HTML files
# saveNetwork(sankey_time_to_event_before , file = "~/BHK lab/EcoTyper_Project/outputs/Dataprocessing_outputs/Plots/Sankey_Before_Time_Event.html")
# saveNetwork(sankey_time_to_event_after, file = "~/BHK lab/EcoTyper_Project/outputs/Dataprocessing_outputs/Plots/Sankey_After_Time_Event.html")

```

```{r sankey with third layer Reponse(RvsNR) }

generate_sankey_with_response <- function(data) {
  # Calculate total patients
  total_patients <- sum(data$Patients, na.rm = TRUE)
  
  # Summarize data for Sankey diagram
  summary_sankey <- data %>%
    group_by(Cancer_Type, Treatments, Response_outcome) %>%
    summarise(Total_Patients = sum(Patients, na.rm = TRUE), .groups = "drop")
  
  # Parse Response_outcome into R, NR, and NA counts
  response_summary <- summary_sankey %>%
    mutate(
      NR = as.numeric(gsub(".*NR: ([0-9]+).*", "\\1", Response_outcome)),
      R = as.numeric(gsub(".*R: ([0-9]+).*", "\\1", Response_outcome)),
      NA_count = as.numeric(gsub(".*NA: ([0-9]+).*", "\\1", Response_outcome)),
      NR = ifelse(is.na(NR), 0, NR),
      R = ifelse(is.na(R), 0, R),
      NA_count = ifelse(is.na(NA_count), 0, NA_count)
    ) %>%
    pivot_longer(cols = c(NR, R, NA_count), names_to = "Response", values_to = "Response_Patients")
  ...
  nodes <- data.frame(
    name = c(
      ...
      paste0(unique(summary_sankey$Treatments), "\n(n=", 
             ...
             "%)"),
      # Response Categories
      paste0(unique(response_summary$Response), "\n(n=", 
             sapply(unique(response_summary$Response), function(resp) sum(response_summary$Response_Patients[response_summary$Response == resp], na.rm = TRUE)), 
             ", ", 
             round(sapply(unique(response_summary$Response), function(resp) 
               sum(response_summary$Response_Patients[response_summary$Response == resp], na.rm = TRUE) / total_patients * 100), 2), 
             "%)")
    )
  )
  ...
  # Treatments to Response Outcome
  links <- links %>%
    bind_rows(
      response_summary %>%
        mutate(
          source = match(paste0(Treatments, "\n(n=", 
                                ...
                                round(sapply(Treatments, function(t) 
                                  sum(summary_sankey$Total_Patients[summary_sankey$Treatments == t]) / total_patients * 100), 2), 
                                "%)"), nodes$name) - 1,
          target = match(paste0(Response, "\n(n=", 
                                sapply(Response, function(resp) sum(response_summary$Response_Patients[response_summary$Response == resp], na.rm = TRUE)), 
                                ", ", 
                                round(sapply(Response, function(resp) 
                                  sum(response_summary$Response_Patients[response_summary$Response == resp], na.rm = TRUE) / total_patients * 100), 2), 
                                "%)"), nodes$name) - 1,
          value = Response_Patients
        ) %>%
        select(source, target, value)
    )
...
# Sankey diagram using Response_outcome as the third layer
sankey_response_before <- generate_sankey_with_response(summary_table)
sankey_response_after <- generate_sankey_with_response(summary_table_after)

sankey_response_before
sankey_response_after

# saveNetwork(sankey_before_response, file = "~/BHK lab/EcoTyper_Project/outputs/Dataprocessing_outputs/Plots/Sankey_Before_Response.html")
# saveNetwork(sankey_after_response, file = "~/BHK lab/EcoTyper_Project/outputs/Dataprocessing_outputs/Plots/Sankey_After_Response.html")
```
