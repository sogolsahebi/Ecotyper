...
# TODO: Make sure all packages, defined directories, chunck names, caption of figures, summary or aims for each script (explained at the begining of script), etc are already used in the script

Base_dir <- "~/BHK lab/PredictIO/PredictIO_EcoTyper/"

# Define input and output directories
output_dir <- file.path(Base_dir, "outputs", "MetaAnalysis_outputs")
input_dir <- file.path(Base_dir, "outputs", "Cibersort_Alldatasets_outputs")

# Load RData files for PFS, OS, and Response (Responders vs. Non-Responders)
load(file.path(input_dir, "All_Association_PFS_Cibersort.RData"))
load(file.path(input_dir, "All_Association_OS_Cibersort.RData"))
load(file.path(input_dir, "All_Association_Response_RvsNR_Cibersort.RData"))
...
kable(pfs_meta_results, 
      ...
      col.names = c("Cell Type", "Coef", "SE", "CI Lower", "CI Upper", "P-value", "I2", "Q P-value", "FDR")) %>%kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
  scroll_box(height = "400px")
```
...
# write.csv(os_meta_results, file = file.path(output_dir, "Meta_analysis_OS_PanCancer.csv"), row.names = FALSE)
...
#TODO: add  visualization figure including volcano for meta results and forestplot for only significant associated cell types. There is forestplot function in package
