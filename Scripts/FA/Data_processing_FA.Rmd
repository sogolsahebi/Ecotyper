...
data_files <- list.files(path = file.path(Base_dir, "data/data_all"), pattern = "\\.rda$", full.names = TRUE)
data_sex_files <- list.files(path = file.path(Base_dir, "data/data_sex"), pattern = "\\.rda$", full.names = TRUE)
...
####################### COMMENT: It should be updated. It does not take into account "response" outcome, if there is time-to-event outcome. As an example Gide has all three outcomes, but only OS/PFS was shown in the table.  ################################
...
############################## COMMENT #############################################
1. Remove item 3 since you already mentioned "only keeping PRE samples" taht can cover step/item 3
2. what do you mean NA patients? 
...
################################# COMMENT ######################################
## Why did you keep both death and alive? 
################################################################################
...
################################### COMMENT #######################################
## This line has an error on line 288: "data$study_base <- sapply(data$study, get_study_base)"
## combined_clin_sex_before does not have column named "study"
###################################################################################
before_df <- merge(
